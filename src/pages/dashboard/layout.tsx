...
import { NavItems } from "../../components/common/nav";
...
function AppSidebar({
  ...
  return (
    <SidebarContent className="flex flex-col h-full ">
      ...
      {isOpen ? (
        <div className="flex flex-col justify-between p-4 border-r border-black/40">
      ...
      <SidebarGroup className="flex-1 overflow-y-auto">
        {isOpen && <SidebarGroupLabel className="sr-only">Application</SidebarGroupLabel>}
        <SidebarGroupContent>
          <SidebarMenu className="flex flex-col gap-2 -mt-4 ">
            {NavItems.map((item) => (
              <SidebarMenuItem key={item.title} >
                <SidebarMenuButton asChild className="hover:bg-[#6D28D9]/40 hover:text-[#7026e6] transition-colors duration-300 ease-in-out">
                  <Link
                    to={item.url}
                    className={`${isOpen ? "px-6" : "px-4"}  flex items-center py-6 rounded hover:bg-[#6D28D9]/40 hover:text-[#7026e6] transition-colors duration-300 ease-in-out`}
                  >
...
function DashboardLayout() {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-dvh lg:h-screen lg:md:overflow-hidden w-full">
      {/* Sidebar */}
        <aside
          className={`hidden md:flex flex-col h-screen shadow-md border-r border-black/30 text-black font-sans bg-primary-100 transition-all duration-300 ${isSidebarOpen ? "w-72" : "w-16"
            }`}
        >
          <SidebarProvider>
            <Sidebar
      ...
      {/* Main content */}
      <main className="w-full bg-gray-50 px-4 lg:px-8 lg:pt-8 min-h-screen grow md:overflow-y-auto box-border lg:min-h-0 transition-all duration-300">
        <div className="relative bg-gray-50">
          <DashboardHeader title={title} titleSpan={titleSpan} />
        </div>
        <Outlet />
      </main>
    </div>
